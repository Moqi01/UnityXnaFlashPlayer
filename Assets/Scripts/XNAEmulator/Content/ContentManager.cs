...
namespace Microsoft.Xna.Framework.Content
{
	public class ContentManager
	{
		...
		public T1 Load<T1>(string asset) where T1 : IDisposable
		{
			Type type = typeof(T1);
            ...
            {
                Texture3D texture = LoadTexture3D(asset);
                return (T1)Convert.ChangeType(texture, type);
            }
            ...
            else if (type == typeof(SoundEffectInstance))
            {
                SoundEffectInstance soundEffect = LoadSoundEffectInstance(asset);
                return (T1)Convert.ChangeType(soundEffect, type);
            }
        ...
        private Texture3D LoadTexture3D(string asset)
        {
            UnityEngine.Texture2D unityTexture = new UnityEngine.Texture2D(2, 2);
            ...
            return new Texture3D(unityTexture);
        }
        ...
        private SpriteFont LoadSpriteFont(string asset)
		{
			...
			{
                //Texture2D LoadTexture2D = Load<Texture2D>(asset);

                SpriteFont spriteFont = new SpriteFont(fontSettings["FontName"], float.Parse(fontSettings["Size"]), float.Parse(fontSettings["Spacing"]), bool.Parse(fontSettings["UseKerning"]), fontSettings["Style"], null);
        ...
        private SoundEffectInstance LoadSoundEffectInstance(string asset)
        {
